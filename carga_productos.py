...
def grabarProd(cod1,cod2,nom,peso,vol):
    ...
    puntero.execute("INSERT INTO Productos VALUES('"+cod1+"','"+cod2+"','"+nom+"',"+peso+","+vol+")")
...
codigoBaseEntry=Entry(frame,textvariable=codigo2,width=6)
codigoBaseEntry.grid(row=1,column=1,sticky="w",padx=5, pady=5)
codigoBaseEntry.config(font="Arial 15")
...
guardarBtn=Button(frame,text="Guardar", command=lambda:grabarProd(codigo1.get(),codigo2.get(),nombre.get(),peso.get(),volumen.get()))
